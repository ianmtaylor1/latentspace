...
for (var in vars) {
  ...
  truth <- gsub("beta", "delta", paste0(var, "_true"))
...
longform$varname <- factor(longform$varname, levels=c("intercept", "column1", "column2", "dyad1", "dyad2"))
...
for (noiselvl in c(1, 2)) {
  for (corlvl in c(0.1, 0.9)) {
    ...
    png(file.path(plotdir, paste0("isba_noise", noiselvl, "_cor", corlvl, ".png")),
