...
for (run in covar.runs) {
  ...
  # Start the MPI cluster here so it's only used during the necessary parts
  cl <- startMPIcluster()
  registerDoMPI(cl)
  
  runresults <- foreach(rep=reps,.combine="rbind") %dopar% {
  ...
  results <- rbind(results,runresults)
}
