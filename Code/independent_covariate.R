...
jobname <- "nodcor"
workers <- 24        # Number of parallel processes to run

covar.runs <- 1:20  # Number of times to generate a set of covariates
reps <- 1:200        # Number of times to generate Y values for each set of covariates
...
results <- data.frame(Run=integer(),
                      Rep=integer(),
                      Xseed=integer(),
                      Yseed=integer(),
                      Z=logical(),
...
for (run in covar.runs) {
  ...
  runresults <- foreach(rep=reps) %dopar% {
  ...
  # Add the run results to our overall data frame
  combinedresults <- runresults[[1]]
  for (i in 2:length(runresults)) {
    combinedresults <- rbind(combinedresults, runresults[[i]])
  }
  
  results <- rbind(results,combinedresults)
}
