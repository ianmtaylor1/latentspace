...
jobname <- "cor-a-X" # How to name the output file
...
results <- foreach(rep=reps, .combine="rbind") %dopar% {
  ...
  Y_binary_noZ <- 1 * (eta > 0) # Data generated WITHOUT unobserved covariate (binary)
  Y_normal_noZ <- eta           # Data generated WITHOUT unobserved covariate (normal)
  ...
  foreach(args=arglists, .combine="rbind") %do% {
    fit <- do.call(ame, args[c("Y","Xdyad","rvar","cvar","dcor","R","plot","print","model","gof","nscan")])
    a_est <- fit$APM
    X1_rmean <- apply(args$Xdyad[,,"X1"],1,"mean",na.rm=TRUE)
    X2_rmean <- apply(args$Xdyad[,,"X2"],1,"mean",na.rm=TRUE)
    X3_rmean <- apply(args$Xdyad[,,"X3"],1,"mean",na.rm=TRUE)
    data.frame(Run=run,
               Rep=rep,
               Xseed=xseed,
               Yseed=yseed,
               ...
               AddRE=args$rvar,
               MulRE=(args$R > 0),
               X1cor=cor(a_est,X1_rmean),
               X2cor=cor(a_est,X2_rmean),
               X3cor=cor(a_est,X3_rmean),
               Xtotcor=cor(a_est, X1_rmean+X2_rmean+X3_rmean),
